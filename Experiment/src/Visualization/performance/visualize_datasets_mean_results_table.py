...
baseline = "one_hot"
path_to_plotting_results = root / "plot/performance/mean" / (baseline + "_without_multi")
...
if __name__ == "__main__":
    ...
    for task_type in task_types:
        ...
        mean_performance_encoder_dataframe = ResultsEvaluator.generate_mean_value_per_column(relative_performance_dataframe)
        mean_performance_encoder_dataframe = pd.DataFrame(mean_performance_encoder_dataframe, index=[0])
        cells = []
        for encoding_ombination in mean_performance_encoder_dataframe.columns:
            cells.append(mean_performance_encoder_dataframe[encoding_ombination])
