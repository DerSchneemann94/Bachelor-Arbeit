...
baseline = "one_hot"
path_to_plotting_results = root / "plot/time/mean/tables" / (baseline + "_without_multi")
...
if __name__ == "__main__":
    ...
    for task_type in task_types:
        ...
        datasets_elapsed_time_dataframe.drop("openml_ids", axis=1, inplace=True)
        datasets_elapsed_time_mean_dataframe = ResultsEvaluator.generate_mean_value_per_column(datasets_elapsed_time_dataframe)
        datasets_elapsed_time_mean_dataframe = pd.DataFrame(datasets_elapsed_time_mean_dataframe, index=[0])
        cells = []
        for enoder_combination in datasets_elapsed_time_mean_dataframe.columns:
            cells.append(datasets_elapsed_time_mean_dataframe[enoder_combination])
        fig = go.Figure(data=[go.Table(
        header=dict(values=list(datasets_elapsed_time_mean_dataframe.columns),
